...
wordlists/%.txt.sorted: wordlists/%.txt
	@echo "Генериране на $@ от $< ..."; cat $< | sort | uniq > $@

wordlists/%.sorted.md5: wordlists/%.sorted
	@echo "MD5 хеширане на $< и записване в $@ ..."; cat $< | perl -e 'use Digest::MD5 md5_hex; while(<>){ chomp; print md5_hex($$_)."\n"; }' > $@

wordlists/%.sorted.sha1: wordlists/%.sorted
	@echo "SHA1 хеширане на $< и записване в $@ ..."; cat $< | perl -e 'use Digest::SHA sha1_hex; while(<>){ chomp; print sha1_hex($$_)."\n"; }' > $@
